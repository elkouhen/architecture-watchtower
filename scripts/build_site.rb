...
css = <<~CSS
  ...
  * { box-sizing:border-box; } body { margin:0; background:var(--bg); color:var(--ink); font:16px/1.65 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
  ...
  article { background:var(--surface); border:1px solid var(--line); border-radius:22px; box-shadow:var(--shadow); padding:clamp(24px,5vw,58px); } article h1 { font-size:clamp(2rem,4vw,4rem); } article h2 { border-top:1px solid var(--line); padding-top:34px; } article h3 { margin-top:30px; } article p,article ul,article ol { max-width:850px; } article li { margin:7px 0; } article p code,article li code,article td code,article th code { background:var(--accent-soft); color:var(--ink); border-radius:5px; padding:2px 5px; } article pre { background:#101827; border-radius:12px; color:#dbeafe; overflow:auto; padding:18px; } article pre code { background:transparent; color:inherit; border-radius:0; padding:0; } .table-wrap { overflow-x:auto; } table { border-collapse:collapse; margin:20px 0; min-width:700px; width:100%; } th,td { border:1px solid var(--line); padding:10px 12px; text-align:left; vertical-align:top; } th { background:var(--accent-soft); color:var(--ink); }
