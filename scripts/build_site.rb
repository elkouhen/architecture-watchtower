...
css = <<~CSS
  ...
  * { box-sizing:border-box; } body { margin:0; background:var(--bg); color:var(--ink); font:16px/1.65 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
  ...
  .shell { margin:0 auto; max-width:1180px; padding:72px 24px 96px; } .hero { align-items:end; border-bottom:1px solid var(--line); display:grid; gap:40px; grid-template-columns:1.4fr .6fr; margin-bottom:54px; padding-bottom:48px; } .eyebrow { color:var(--accent); font-size:.72rem; font-weight:800; letter-spacing:.14em; text-transform:uppercase; } h1,h2,h3 { letter-spacing:-.045em; line-height:1.1; } h1 { font-size:clamp(2.8rem,6vw,5.9rem); margin:14px 0 20px; max-width:850px; } h2 { font-size:clamp(1.8rem,3vw,2.5rem); margin-top:48px; } h3 { font-size:1.1rem; } .hero p { color:var(--muted); font-size:1.08rem; max-width:650px; } .signal-card { align-self:end; background:var(--surface); border-left:3px solid var(--accent); border-radius:0 14px 14px 0; box-shadow:none; color:var(--ink); padding:8px 0 8px 22px; } .signal-card strong { display:inline-block; font-size:2.6rem; letter-spacing:-.08em; margin-right:8px; } .signal-card span { color:var(--muted); margin-right:18px; }
