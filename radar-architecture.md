...
En mode `watchtower:orchestrated`, le coût en contexte est une contrainte de qualité :
...
1. **Scanner.** Traite d’abord les signaux arrivés à échéance. Construis ensuite un petit ensemble de sources primaires couvrant AWS, GCP et IA : nouveautés et sécurité pour AWS, nouveautés et sécurité pour GCP, changelog et dépréciations pour l’IA. Mutualise une source lorsqu’elle couvre plusieurs voies. Vise six à huit sources de contrôle, sans en faire un plafond lorsque la couverture l’exige, et consulte au moins un et au maximum deux flux de découverte open source. Cette étape de découverte n’est jamais sautée pour économiser du budget : son coût est marginal et son absence prive le radar de la moitié de son objectif. Pour chaque source, reprends à `last_success`, arrête la lecture après le dernier élément déjà enregistré et ne tente qu’un seul fallback en cas d’échec ou de périmètre insuffisant.
2. **Filtrer.** Pour chaque élément détecté, pose successivement trois questions : comporte-t-il un changement nouveau ou substantiel ; modifie-t-il une décision d’architecture, de sécurité, d’exploitation ou de coût ; possède-t-il une preuve primaire précise ? Une réponse négative arrête immédiatement l’analyse du candidat.
...
Présente normalement cinq à sept sujets suffisamment qualifiés, mais accepte de zéro à quatre sujets lors d’un cycle calme ; aller jusqu’à dix seulement si chaque sujet ajoute une décision distincte ou pour préserver une alerte critique. Le filtre en trois questions limite naturellement les candidats : n’impose pas de quota d’exploration et arrête immédiatement l’analyse d’un doublon, d’un sujet hors périmètre, d’une annonce non vérifiable ou d’un élément sans impact décisionnel. À priorité comparable, privilégie les dernières 48 heures, puis sept jours et trente jours. Un seul signal crédible suffit pour une découverte ; `traction étayée` exige une deuxième preuve indépendante et une source primaire suffisante. La maturité reste distincte de la popularité.
