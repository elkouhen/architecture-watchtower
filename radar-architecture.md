...
Avec `--replace`, réécris uniquement le rapport existant à partir de son manifest déjà sélectionné. Ne relance ni collecte, ni qualification, ni sélection, et ne modifie aucun registre, index ou manifest : la déduplication ne doit jamais opposer un livrable historique à lui-même. Une nouvelle observation ou un changement de sélection exige une nouvelle exécution datée, pas `--replace`.

Lorsque l’orchestrateur injecte aussi `watchtower:isolate-v1`, la phase de qualification est obligatoirement décomposée en collecte locale, analyses isolées et agrégation. Chaque URL canonique est téléchargée une seule fois dans un cache temporaire non commité, puis normalisée sans perdre les titres, dates, versions, tableaux, blocs de code ou passages nécessaires à la preuve. Chaque document, ou petit lot homogène explicitement borné, est analysé dans un nouveau contexte sans contenu des documents précédents. L’analyse reçoit uniquement la source, ses voies, sa fenêtre de delta et l’index local minimal nécessaire à la déduplication ; elle retourne un résultat structuré et ne modifie aucun fichier. L’agrégateur valide ces résultats, construit le manifest complet et laisse au script Ruby la sélection. La rédaction ne reçoit ensuite que la sélection, les preuves associées et les enregistrements locaux ciblés. Le contrat détaillé et ses critères d’activation figurent dans `docs/contrats-veille.md`.

En l’absence du marqueur `watchtower:isolate-v1`, conserve le protocole orchestré actuel : ne simule pas une isolation que le runner ne garantit pas et ne lance pas toi-même de sous-processus Codex.
...
Publie normalement 5 à 7 sujets, au plus 10 sauf alerte critique. Pour atteindre cinq sujets, élargis la qualification aux deltas primaires déjà collectés et aux pistes de découverte pertinentes, sans élargir la fenêtre de 90 jours ni abaisser les trois filtres. La déduplication prévaut toujours : ne republie un sujet déjà observé dans les 90 jours que si une évolution substantielle vérifiée modifie son statut, son risque, son échéance ou sa recommandation ; son type est alors `Mise à jour` et le delta est explicite. Si moins de cinq sujets remplissent ces conditions, publie seulement les sujets éligibles et inscris `Exception minimum de sujets : <motif factuel>` dans `Sujets écartés`. Ne complète jamais le quota avec un simple suivi de signal actif. Vise 12 à 20 candidats ; si les flux en fournissent moins, examine-les tous et indique le rendement dans `Sujets écartés`.
