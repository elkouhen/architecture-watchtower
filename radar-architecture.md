...
Le radar doit contenir **au moins 50 % de nouveaux projets, services ou patterns** : des sujets jamais présentés dans les rapports ou signaux des 90 derniers jours. Pour un radar de huit à dix sujets, vise donc quatre à cinq nouveautés minimum. Les mises à jour de produits déjà suivis — releases, correctifs, changements de version, fonctionnalités ou incidents — sont plafonnées à la moitié du radar. Une nouveauté reste soumise à la déduplication stricte : un simple renommage, fork sans différenciation ou nouvelle version d’un sujet déjà suivi n’est pas une nouveauté. Si suffisamment de nouveautés qualifiées ne sont pas disponibles, réduis le nombre total de sujets et explique le manque dans `Sujets écartés` ; ne compense pas avec des mises à jour supplémentaires. La vue d’ensemble doit indiquer le statut `Nouveau` ou `Mise à jour` dans le pitch rapide ou la fiche, afin que l’équilibre soit immédiatement vérifiable.
