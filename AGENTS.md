...
Chaque nouveau radar possède aussi un manifest d’exécution sous `state/radar-runs/AAAA-MM-JJ.yaml`. Ce registre technique contient les candidats, les preuves et la sélection déterministe ; il ne constitue pas un livrable supplémentaire.
...
1. Lire la consigne source concernée et `docs/contrats-veille.md` avant de lancer la recherche. Utiliser le contrat version 2 pour tout nouveau rapport ; conserver le contrat historique des archives non corrigées. Pour un radar orchestré, leur contenu et la projection locale `watchtower:prepared-context` sont injectés par `scripts/run_radar.rb` : ne pas les relire sur disque sauf incohérence précise. La qualification écrit le manifest, le script calcule la sélection, puis un second tour rédige uniquement cette sélection.
...
- Le radar contient au minimum cinq sujets qualifiés, normalement cinq à sept et au plus dix, sauf dépassement critique motivé. Si moins de cinq sujets franchissent réellement les trois filtres après élargissement aux deltas et pistes déjà collectés, publier les sujets éligibles avec une exception factuelle explicite ; ne jamais inventer ou forcer un sujet faible. Il vise toujours 33 % de nouveaux projets open source parmi les sujets retenus, arrondis à l’entier supérieur. Ce quota ne doit jamais forcer l’ajout d’un sujet ni masquer une vulnérabilité, un incident, une dépréciation, un changement incompatible ou une évolution AWS/GCP/IA à fort impact ; les mises à jour de produits connus ne sont pas plafonnées.
