...
La cible minimale est de cinq sujets. Si la sélection contient moins de cinq candidats après qualification complète, le bloc `selection` porte `minimum_exception` avec le motif factuel issu du rendement de collecte ; le rapport reprend exactement `Exception minimum de sujets : <minimum_exception>`. Lorsque cinq sujets ou plus sont retenus, `minimum_exception` vaut `null`. Cette exception ne rend jamais éligible un candidat ayant échoué l’un des trois filtres.
